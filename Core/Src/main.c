...
#define HALL_OverSample 	3
	...
	#define TIM_DELAY	5
		/**
		 * @def HighState
		 * @brief The output of 2 MOSFET To Generate High gate drivers pins
		 * @note HIN is the first bit and the LIN Bar is the other
		 */
	#define HighState 	0b01
		/**
		 * @def LowState
		 * @brief The output of 2 MOSFET To Generate Low gate drivers pins
		 * @note HIN is the first bit and the LIN Bar is the other
		 */
	#define LowState  	0b00
		/**
		 * @def FloatState
		 * @brief The output of 2 MOSFET To Generate Float gate drivers pins
		 * @note HIN is the first bit and the LIN Bar is the other
		 */
	#define FloatState	0b10
		/**
		 * @def MAX_PWM
		 * @brief define the maximum duty cycle value that adc reach to avoid the fault in pwm
		 * @note Do not exceed this value to avoid faults
		 */
	#define MAX_PWM		100
		/**
			 * @def MIN_PWM
...
/**
	...
	/**
	 * @var     :Counter
	 * @brief   :Global variable storing the Counter Value that change the current step of the motor .
	 * @warning :Do not modify this value directly.
	 * @Type    :uint8_t
	 */
	uint8_t Counter;
